...
### ~~Persistent Routine Sessions~~ ✓ Implemented
`session: persistent` in routine YAML frontmatter. Standalone session per
routine, stored in `~/.ollim-bot/state/routine_sessions/<routine-id>`.
Agent-driven compaction via `compact_session` MCP tool (deferred execution).
Skip guard prevents concurrent fires. Session cleanup on routine removal.
